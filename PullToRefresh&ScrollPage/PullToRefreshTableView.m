...
-(id)initWithFrame:(CGRect)frame withType:(PullToRefreshTableViewType)aType
{
    self = [super initWithFrame:frame style:UITableViewStylePlain];
    if (self) {
        //        selfFrame = frame;
        NSLog(@"AppTableView initWithFrame 0");
        tableType = aType;
        if(tableType == withStateViews)
        {
...
- (void)tableViewDidDragging{
    ...
    if(tableType == withoutStateViews)
    {
        return;
    }
